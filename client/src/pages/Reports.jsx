  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-slate-900/40 p-4 border border-slate-800 rounded-xl glass-panel">
        <div>
          <h2 className="text-base font-semibold text-slate-200">Inventory Valuation & Reports</h2>
          <p className="text-xs text-slate-500 mt-0.5">Download full audits and valuation balance logs.</p>
        </div>
        
        <div className="flex gap-2 w-full sm:w-auto">
          <button
            onClick={handleDownloadExcel}
            className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-4 py-2 bg-emerald-950 text-emerald-400 border border-emerald-900/60 hover:bg-emerald-900/20 rounded-lg text-sm font-semibold transition cursor-pointer"
          >
          ...
          <button
            ...
            className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-4 py-2 bg-red-950 text-red-400 border border-red-900/60 hover:bg-red-900/20 rounded-lg text-sm font-semibold transition cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-6 rounded-xl border border-slate-800 bg-slate-900/50 glass-panel">
          <span className="text-xs text-slate-500 font-semibold block uppercase tracking-wider">Total Catalog Products</span>
          <span className="text-2xl font-extrabold block mt-2 text-slate-100">{summary?.totalProducts || 0}</span>
        ...
        <div className="p-6 rounded-xl border border-slate-800 bg-slate-900/50 glass-panel">
          <span className="text-xs text-slate-500 font-semibold block uppercase tracking-wider">Total Items Stocked</span>
          <span className="text-2xl font-extrabold block mt-2 text-slate-100">{summary?.totalItems || 0}</span>
          <div className="flex items-center gap-1 text-[10px] text-slate-500 mt-2">
        ...
        <div className="p-6 rounded-xl border border-slate-800 bg-slate-900/50 glass-panel">
          <span className="text-xs text-slate-500 font-semibold block uppercase tracking-wider">Gross Stock Value</span>
          <span className="text-2xl font-extrabold block mt-2 text-emerald-400">{formatCurrency(summary?.totalValueRetail || 0)}</span>
          <div className="flex items-center gap-1 text-[10px] text-slate-500 mt-2">
        ...
        <div className="p-6 rounded-xl border border-slate-800 bg-slate-900/50 glass-panel">
          <span className="text-xs text-slate-500 font-semibold block uppercase tracking-wider">Net Stock Value (Cost)</span>
          <span className="text-2xl font-extrabold block mt-2 text-blue-400">{formatCurrency(summary?.totalValueCost || 0)}</span>
          <div className="flex items-center gap-1 text-[10px] text-slate-500 mt-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 p-6 rounded-xl border border-slate-800 bg-slate-900/50 glass-panel flex flex-col justify-between">
          <div>
            <h3 className="text-base font-semibold text-slate-200 mb-2">Estimated Margin Breakdown</h3>
            <p className="text-xs text-slate-500 leading-relaxed">Calculated difference between selling price and acquisition cost of currently stocked items. Represents unrealized profit margin.</p>
          </div>

          <div className="my-6 space-y-4">
            <div className="flex justify-between items-center text-sm">
              <span className="text-slate-400">Total Est. Profit</span>
              <span className="font-bold text-emerald-400">{formatCurrency(summary?.estimatedProfit || 0)}</span>
            </div>
          ...
          <div className="p-3.5 bg-slate-950/60 border border-slate-800 rounded-lg text-xs text-slate-400 flex items-start gap-2.5">
        ...
        <div className="lg:col-span-2 p-6 rounded-xl border border-slate-800 bg-slate-900/50 glass-panel">
